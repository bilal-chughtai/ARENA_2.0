...
def apply_scale(resid: Float[Tensor, "batch=1 seq d_model"], flip_dir: Float[Tensor, "d_model"], scale: int, pos: int):
    ...
    resid_at_pos = resid[0, pos]
    alpha = einops.einsum(resid_at_pos, flip_dir, "d_model, d_model ->")
    orth_resid = resid_at_pos - alpha * flip_dir
    resid[0, pos] = orth_resid - alpha * scale * flip_dir
    return resid
...
def calculate_attn_and_mlp_probe_score_contributions(
    ...
    attn_activations =  t.stack([focus_cache["attn_out", l] for l in range(layer+1)])[:, game_index, move]
    mlp_activations = t.stack([focus_cache["mlp_out", l] for l in range(layer+1)])[:, game_index, move]
    
    attn_contributions = einops.einsum(attn_activations, my_probe, "layers d_model, d_model rows cols -> layers rows cols")
    mlp_contributions = einops.einsum(mlp_activations, my_probe, "layers d_model, d_model rows cols -> layers rows cols")
...
def calculate_accumulated_probe_score(
    ...
    activations = focus_cache["resid_post", layer][game_index, move]
    
    contributions = einops.einsum(activations, my_probe, "d_model, d_model rows cols -> rows cols")
...
def get_w_in(
    ...
    '''
    w = model.W_in[layer][:, neuron].clone().detach()
    if normalize:
        w = w / w.norm()
    return w
...
def get_w_out(
    ...
    If normalize is True, the weights are normalized to unit norm.
    '''
    w = model.W_out[layer][neuron].clone().detach()
    if normalize:
        w = w / w.norm()
    return w
...
def calculate_neuron_input_weights(
    ...
    return einops.einsum(w, probe, "d_model, d_model rows cols -> rows cols")
